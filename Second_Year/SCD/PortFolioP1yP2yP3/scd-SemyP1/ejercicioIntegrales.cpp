...
double funcion_hebra( long i )
{
   ...
   if(i >= 0 && i < n){
	   for (long k = i*(m/n); k < (i+1)*(m/n); k++)
		   suma_parcial += f( (k+double(0.5)) / m);
	}
...
{
   future<double> sumasparciales[n];
   double suma=0.0;

   for( int i=0; i<n; i++){
	   sumasparciales[i] = async (launch::async, funcion_hebra, i);
   }
   for(int i=0; i<n; i++){
	   suma += sumasparciales[i].get();
   }
