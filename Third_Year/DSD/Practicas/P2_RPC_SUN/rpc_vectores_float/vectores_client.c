...
vectores_prog_1(char *host, miVector var1, char oper, miVector var2)
{
	...
	printf("(%f", result->vector.vector_val[0]);
	for(int i = 1; i < result->vector.vector_len; i++){
		printf(", %f", result->vector.vector_val[i]);
	}
	printf(")\n");

	//xdr_free(xdr_miVector, &result);
#ifndef	DEBUG
	clnt_destroy (clnt);
#endif	 /* DEBUG */
}
...
main (int argc, char *argv[])
{
	...
	var1.vector.vector_val = (float*)malloc(num_elem * sizeof(float));
	var2.vector.vector_len = num_elem;
	var2.vector.vector_val = (float*)malloc(num_elem * sizeof(float));
