...
void mouseWheel(int wheel, int direction, int x, int y){
	if (escena!=NULL)
   escena->mouseWheel(wheel, direction, x, y);
...
int main( int argc, char **argv )
{
   ...
   glutCreateWindow("Practicas IG: Raul Rodriguez Perez (raulrguez@correo.ugr.es)");
   ...
   cout << endl << "\t" << "Bienvenido al menú interactivo de la practica 1" << endl;
   cout << "Selecciona el modo que quiera utilizar: " << endl << "'O' : Seleccion de objeto" << endl 
                           << "'V' : Modo de visualizacion" << endl << "'D' : Modo de dibujado" << endl
                           << "'K' : Modo con/sin tapas" << endl << "'M' : Modo Manual" << endl
                           << "'U' : Animacion auto" << endl << "'Q' : Salir" << endl;
